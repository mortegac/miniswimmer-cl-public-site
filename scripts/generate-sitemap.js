...
// NOTE: src/app/sitemap.ts is now the single source of truth for the sitemap.
// This script is kept for legacy reference but no longer writes public/sitemap.xml
// to avoid overriding Next.js's dynamic sitemap route.
console.log(
  "ℹ️  Sitemap is generated by src/app/sitemap.ts (Next.js App Router).",
);
console.log("ℹ️  Skipping legacy sitemap generation script.");
process.exit(0);
...
function generateSitemap() {
  ...
  const allRoutes = [...staticRoutes, ...cleanedRoutes].filter(
    (route, index, arr) => arr.indexOf(route) === index,
  );
  ...
  const sitemapEntries = allRoutes.map((route) => {
    ...
    if (route === "" || route === "/") {
      ...
      route.includes("servicios") ||
      route.includes("pricing") ||
      route.includes("precios")
    ) {
      priority = 0.9;
      changeFrequency = "weekly";
...
${sitemapEntries
  ...
  console.log(
    `✅ Sitemap generado exitosamente con ${sitemapEntries.length} rutas`,
  );
